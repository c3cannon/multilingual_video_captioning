...
def build_vocab(vids, params):
    ...
    for vid, caps in vids.items():
        ...
        for cap in caps:
            ws = list(cap)  #re.sub(r'[.!,;?]', ' ', repr(cap)).split('\\')
            caption = [
...
def main(params):
    videos = json.load(open(params['input_json'], 'r'))#['enCap']
    video_caption = {}
    for i in videos:
        if i['videoID'] not in video_caption.keys():
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', type=str, default='vatex_validation.json',
                        help='VATEX training data json')

    parser.add_argument('--info_json', default='info_val_chinese.json',
                        help='info about iw2word and word2ix')
    parser.add_argument('--caption_json', default='caption_val_chinese.json', help='caption json file')
