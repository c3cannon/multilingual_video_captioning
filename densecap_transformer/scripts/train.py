...
def get_dataset(args):
    # process text
    text_proc, raw_data = get_vocab_and_sentences(args.train_dataset_file, args.val_dataset_file, args.max_sentence_len)
    ...
    print("TRAIN DATA FOLDER:", args.train_data_folder)
...
def train(epoch, model, optimizer, train_loader, vis, vis_window, args):
    ...
    for train_iter, data in enumerate(train_loader):
        ...
        # reg_loss = model.reg_loss(pred_offsets, gt_offsets) * args.reg_weight
        ...
        print('iter: [{}/{}], training loss: {:.4f}, '
              ...
              'data time: {:.4f}s, total time: {:.4f}s'.format(
            train_iter, nbatches, total_loss.data.item(), cls_loss.data.item(),
            sent_loss.data.item(), mask_loss.data.item(),
            total_grad_norm,
            t_model_start - t_iter_start,
            t_model_end - t_iter_start
        ), end='\r')
