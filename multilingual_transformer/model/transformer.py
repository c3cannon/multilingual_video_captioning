...
class Transformer(nn.Module):
    ...
    def forward_one_step(self, state, current_word, v_mean, image_feats):
        ...
        # print("state after concat shape:", state.shape)
        # print("state_with_pos shape:", state_with_pos.shape)
        attended_words = self.word_attention(state_with_pos, v_mean)
...
class Sentence(object):
    ...
    def extract_sentence(self):
        ...
        for w in self.words:
            idx = w.max(1)[1].item()
            sentence.append(self.vocab.itos[idx])
