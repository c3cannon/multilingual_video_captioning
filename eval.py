...
def convert_data_to_coco_scorer_format(data_frame, language):
    ...
    if language == 'english':
        ...
        for row in zip(data_frame["chCap"], data_frame["videoID"]):
            for cap in row[0]:
                if row[1] in gts:
                    gts[row[1]].append(
                        {'image_id': row[1], 'cap_id': len(gts[row[1]]), 'caption': cap})
...
def test(model, crit, dataset, vocab, opt):
    ...
    with open(os.path.join(opt["results_path"], "chinese_LSTM_OPT_epoch601_scores.txt"), 'a') as scores_table:
        scores_table.write(json.dumps(results[0]) + "\n")
    with open(os.path.join(opt["results_path"],
                           opt["model"].split("/")[-1].split('.')[0] + "_chinese_LSTM_OPT_epoch601.json"), 'w') as prediction_results:
