    ...
    for epoch in range(opt["epochs"]):
        ...
        if epoch % 2 == 1:
            ...
            if opt["self_crit_after"] != -1 and epoch >= opt["self_crit_after"]:
                sc_flag = True
                init_cider_scorer(opt["english_tokens"])
            else:
                sc_flag = False
...
def main(opt):
    ...
    opt["vocab_size"] = engDataset.get_vocab_size() + chinDataset.get_vocab_size() #must be the total vocab size
