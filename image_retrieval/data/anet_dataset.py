...
def anet_collate_fn(batch_lst):
    ...
    for batch_idx in range(batch_size):
        ...
        lengths[batch_idx] = l + 1 # +1 for <eos> token
        img_batch[batch_idx,:] = img_feat
        sentence_batch[batch_idx] = sentence_idx[1:] # skip <sos> token
        video_prefixes[batch_idx] = video_prefix
        sentences[batch_idx] = sentence

    # + 1 for <eos> token
    # lengths = [len(sent.split()) + 1 for sent in sentences]
    ...
    sort_idx = torch.from_numpy(np.flip(np.argsort(lengths), axis=0).copy())
    sorted_sentences = [sentences[i] for i in sort_idx]
    sorted_vid_prefixes = [video_prefixes[i] for i in sort_idx]
    sorted_lengths = [lengths[i] for i in sort_idx]
    # sorted_sent_batch = [sentence_batch[i] for i in sort_idx]
    sorted_sent_batch = sentence_batch[sort_idx]
    img_batch = img_batch[sort_idx]

    return (img_batch, sorted_sent_batch, sorted_sentences,
        sorted_vid_prefixes, sorted_lengths)
